...
{
  var options = {};
  var _inbound = _source.val();
  var _length = _inbound.length;
  (_length > 1024) ? options.cquery = _inbound.substr(0,1023) : options.cquery = _inbound;
...
function displayNLC(_target, _results)
{
  ...
  {
    (function(_idx2, data)
      {  _cStr = data[_idx2]["class_name"];
        target.append("<tr><td style='width: 60%'>"+_cStr +"</td><td>"+data[_idx2]["confidence"]+"</td></tr>");})
      (_idx, _results)
    _idx++;
  }
  target.append("</table>");
